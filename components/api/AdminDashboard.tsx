...
import { 
  ...
  BarChart3, TrendingUp, Users2, Clock3, UserMinus, UserCheck, Search as SearchIcon
} from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentUser, onPlay, onLogout, onUpdateCounts }) => {
  ...
  // Real-time Analytics Listener (Updated to listen to profiles)
  useEffect(() => {
      if (activeTab === 'statistics') {
          // Listen to 'profiles' updates (when last_seen changes)
          const channel = supabase.channel('admin_analytics_presence')
              .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, () => {
                  fetchStatistics(true); // Silent update when a user heartbeat occurs
              })
              .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'analytics_logs' }, () => {
                  // Also refresh if a new log comes in
  ...
  const fetchStatistics = async (silent = false) => {
      try {
          ...
          // Fetch Active Users List (RPC returns setof profiles)
          const { data: onlineUsers, error: usersError } = await supabase.rpc('get_online_users');
          if (!usersError && onlineUsers) {
              setActiveUsersList(onlineUsers);
          }

          // Detailed Logs Table (Last 100)
          let logsQuery = supabase.from('analytics_logs').select('*, profiles(name, avatar)').order('created_at', { ascending: false }).limit(100);
          if (logFilterUser) {
              logsQuery = supabase.from('analytics_logs').select('*, profiles(name, avatar)').eq('user_id', logFilterUser).order('created_at', { ascending: false }).limit(100);
          }
  ...
  const handleDeleteSelectedLogs = async () => {
      ...
      const confirmed = window.confirm(`هل أنت متأكد من حذف ${selectedLogIds.size} سجلات نهائياً؟`);
      if (!confirmed) return;
      setLoading(true);
      try {
          const { error } = await supabase.from('analytics_logs').delete().in('id', Array.from(selectedLogIds));
  ...
  const handleCleanOldLogs = async () => {
      ...
      setLoading(true);
      try {
          const { error } = await supabase.rpc('clean_analytics_logs');
          if (error) throw error;
  ...
  const handleResetLogs = async () => {
      ...
      setLoading(true);
      try {
          const { error } = await supabase.rpc('reset_analytics_logs');
          if (error) throw error;
              ...
              {activeTab === 'statistics' && (
                  <div className="max-w-6xl mx-auto space-y-8 animate-fadeIn">
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
                          <div className={`bg-neutral-900/40 p-4 rounded-[1.5rem] border transition-all relative overflow-hidden group cursor-pointer lg:col-span-1 ${showActiveList ? 'border-green-500/50 bg-green-900/10' : 'border-white/5'}`} onClick={() => setShowActiveList(!showActiveList)}>
                              ...
                              <div className="flex justify-between items-start mb-2">
                                  ...
                                  <span className="text-green-500 text-[9px] font-black bg-green-500/10 px-2 py-0.5 rounded">مباشر</span>
                              </div>
                          ...
                          <div className="bg-neutral-900/40 p-4 rounded-[1.5rem] border border-white/5 relative overflow-hidden lg:col-span-1">
                              <div className="flex justify-between items-start mb-2"><Clock size={18} className="text-blue-400"/></div>
                              <h3 className="text-2xl font-black text-white mb-1">{statsData.active1h}</h3>
                              <p className="text-[8px] text-gray-500 font-bold uppercase">خلال ساعة</p>
                          </div>

                          <div className="bg-neutral-900/40 p-4 rounded-[1.5rem] border border-white/5 relative overflow-hidden lg:col-span-1">
                              <div className="flex justify-between items-start mb-2"><Clock size={18} className="text-indigo-400"/></div>
                              <h3 className="text-2xl font-black text-white mb-1">{statsData.active5h}</h3>
                              <p className="text-[8px] text-gray-500 font-bold uppercase">5 ساعات</p>
                          </div>
                          <div className="bg-neutral-900/40 p-4 rounded-[1.5rem] border border-white/5 relative overflow-hidden lg:col-span-1">
                              <div className="flex justify-between items-start mb-2"><Clock size={18} className="text-purple-400"/></div>
                              <h3 className="text-2xl font-black text-white mb-1">{statsData.active12h}</h3>
                              <p className="text-[8px] text-gray-500 font-bold uppercase">12 ساعة</p>
                          </div>
                          <div className="bg-neutral-900/40 p-4 rounded-[1.5rem] border border-white/5 relative overflow-hidden lg:col-span-1">
                              <div className="flex justify-between items-start mb-2"><Calendar size={18} className="text-brand"/></div>
                              <h3 className="text-2xl font-black text-white mb-1">{statsData.activeToday}</h3>
                              <p className="text-[8px] text-gray-500 font-bold uppercase">اليوم</p>
                          </div>
                          <div className="bg-neutral-900/40 p-4 rounded-[1.5rem] border border-white/5 relative overflow-hidden lg:col-span-1">
                              <div className="flex justify-between items-start mb-2"><TrendingUp size={18} className="text-orange-400"/></div>
                              <h3 className="text-2xl font-black text-white mb-1">{statsData.activeWeek}</h3>
                              <p className="text-[8px] text-gray-500 font-bold uppercase">الأسبوع</p>
                          </div>
                      </div>

                      {/* --- Active Users List --- */}
                      {showActiveList && (
                          <div className="bg-neutral-900/80 p-6 rounded-[2rem] border border-green-500/20 animate-slideDown shadow-xl">
                              <h4 className="text-green-400 font-black text-sm mb-4 flex items-center gap-2"><Users size={16}/> المتصلون حالياً (آخر دقيقتين)</h4>
                              {activeUsersList.length === 0 ? (
                                  <p className="text-gray-500 text-xs">لا يوجد أعضاء متصلون حالياً</p>
                              ) : (
                                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                      {activeUsersList.map((u: any) => (
                                          <div key={u.id} className="bg-black/40 p-3 rounded-xl flex items-center gap-3 border border-white/5">
                                              <div className="w-8 h-8 rounded-full bg-neutral-800 overflow-hidden relative">
                                                  {u.avatar ? <img src={u.avatar} className="w-full h-full object-cover" alt="avatar"/> : <UserIcon size={16} className="text-gray-500 m-auto mt-2"/>}
                                              ...
                                              <div className="min-w-0">
                                                  ...
                                                  <p className="text-[9px] text-gray-500 truncate">{u.role === 'admin' ? 'مدير' : 'طالب'}</p>
                                              </div>
                                          </div>
                                      ))}
                                  </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          {/* Log Table */}
                          <div className="lg:col-span-2 bg-neutral-900/60 p-8 rounded-[2.5rem] border border-white/5 shadow-xl">
                              <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                                  <h3 className="text-lg font-black text-white flex items-center gap-2"><FileText size={20} className="text-blue-400"/> سجل النشاطات المفصل</h3>
                                  <div className="flex gap-2">
                                      <div className="relative">
                                          <input 
                                            ...
                                            onKeyDown={e => e.key === 'Enter' && fetchStatistics()}
                                            className="bg-black border border-white/10 rounded-xl py-2 px-4 pl-10 text-xs text-white w-48 focus:border-blue-500 outline-none"
                                          />
                                          <SearchIcon size={14} className="absolute left-3 top-2.5 text-gray-500"/>
                                      </div>
                                      <button onClick={() => fetchStatistics()} className="p-2 bg-white/5 rounded-xl hover:bg-white/10"><RefreshCw size={16}/></button>
                                  </div>
                              </div>
                              ...
                              <div className="overflow-x-auto max-h-[400px] custom-scrollbar">
                                  <table className="w-full text-right">
                                      <thead>
                                          <tr className="border-b border-white/10 text-gray-500 text-[10px] font-black uppercase tracking-widest sticky top-0 bg-neutral-900 z-10">
                                              <th className="pb-4 pr-4 w-10"><input type="checkbox" onChange={handleSelectAllLogs} checked={selectedLogIds.size > 0 && selectedLogIds.size === recentLogs.length} className="accent-brand"/></th>
                                      ...
                                      <tbody className="text-xs font-bold text-gray-300">
                                          {recentLogs.map(log => (
                                              <tr key={log.id} className="group hover:bg-white/5 transition-colors border-b border-white/5 last:border-0">
                                                  <td className="py-3 pr-4"><input type="checkbox" checked={selectedLogIds.has(log.id)} onChange={() => handleSelectLog(log.id)} className="accent-brand"/></td>
                                                  <td className="py-3 ltr font-mono text-[10px] opacity-70">{new Date(log.created_at).toLocaleTimeString()}</td>
                                                  <td className="py-3">
                                                      <div className="flex items-center gap-2">
                                                          {log.profiles?.avatar ? <img src={log.profiles.avatar} className="w-5 h-5 rounded-full"/> : <UserIcon size={12} className="text-gray-500"/>}
                                                          <span className={log.user_id ? 'text-white' : 'text-gray-600'}>{log.profiles?.name || 'زائر'}</span>
                                                      </div>
                                                  </td>
                                                  <td className="py-3">
                                                      <span className={`px-2 py-1 rounded-lg text-[9px] ${log.event === 'page_view' ? 'bg-blue-500/10 text-blue-400' : 'bg-purple-500/10 text-purple-400'}`}>{log.page}</span>
                                                  </td>
                                              </tr>
                                          ))}
                                          {recentLogs.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-gray-500">لا توجد سجلات حديثة</td></tr>}
                                      </tbody>
                                  </table>
                              </div>
                              ...
                              <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-center">
                                  <div className="flex gap-2">
                                      <button 
                                        ...
                                        className="px-4 py-2 bg-white/5 hover:bg-white/10 text-gray-300 rounded-xl font-bold text-[10px] flex items-center gap-2 transition-all"
                                      >
                                          <Trash2 size={14}/> تنظيف القديم (&gt;7 أيام)
                                      </button>
                                      <button 
                                        onClick={handleResetLogs}
                                        className="px-4 py-2 bg-red-900/20 hover:bg-red-900/40 text-red-400 rounded-xl font-bold text-[10px] flex items-center gap-2 transition-all"
                                      >
                                          <AlertTriangle size={14}/> تصفير شامل
                                      </button>
                                  </div>
                                  <button 
                                    onClick={handleDeleteSelectedLogs}
                                    disabled={selectedLogIds.size === 0 || loading}
                                    className="px-6 py-2 bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl font-black text-xs flex items-center gap-2 transition-all shadow-lg active:scale-95"
                                  >
                                      {loading ? <Loader2 className="animate-spin" size={14}/> : <Trash2 size={14}/>}
                                      <span>حذف ({selectedLogIds.size})</span>
                                  </button>
                              </div>
                          </div>

                          {/* Top Pages & Stats */}
                          <div className="bg-neutral-900/60 p-8 rounded-[2.5rem] border border-white/5 shadow-xl flex flex-col h-full">
                              <h3 className="text-lg font-black text-white mb-6 flex items-center gap-2"><MapIcon size={20} className="text-brand"/> الأقسام الأكثر زيارة</h3>
                                  ...
                                  {statsData.topPages.map((page, idx) => (
                                      <div key={idx} className="flex items-center justify-between p-4 bg-black/40 rounded-2xl border border-white/5">
                                          <div className="flex items-center gap-3 overflow-hidden">
                                              ...
                                              <span className="text-white text-xs font-bold truncate max-w-[150px]" dir="ltr">{page.page}</span>
                                          </div>
                                          <span className="bg-white/10 text-white px-3 py-1 rounded-lg text-xs font-black">{page.count}</span>
                                      </div>
                                  ))}
                                  {statsData.topPages.length === 0 && <p className="text-center text-gray-500 text-xs">لا توجد بيانات كافية</p>}
                              </div>
                          </div>
                      </div>
                  </div>
